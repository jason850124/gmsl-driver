...
//need to check
struct regmap_config max96724_i2c_regmap = 
{
    .reg_bits = 16,
    .val_bits = 8,
    .max_register = 0x1f00,
...
{   
    ...
    //need to check (get data from dt and compare with our info table)
    ...
    //need to check init regmap method and is_err/ptr_err functions
    priv->regmap = devm_regmap_init_i2c(client, &max96724_i2c_regmap);
    if(IS_ERR(priv->regmap))
        return PTR_ERR(priv->regmap);

    //need to check this function and dev/&client->dev
    priv->gpiod_enable = devm_gpiod_get_optional(dev, "enable", GPIOD_OUT_HIGH);
    if(IS_ERR(priv->gpiod_enable))
        return PTR_ERR(priv->gpiod_enable);

    //need to check sleep function, and need to check the spec
    if(priv->gpio_enable)
        usleep_range(4000, 5000);


    //need to check get_optional function / cansleep; and check the spec
    ...
    //need to check whole assignments
    *ops = max96724_ops;
...
static max96724_remove(struct i2c_client *client)
{
